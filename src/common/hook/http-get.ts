...
export const useHttpGetBlob = <T = Blob, E = unknown, C = unknown>(
    ...
    useEffect((): () => void => {
        ...
        return (): void => { blobUrl && URL.revokeObjectURL(blobUrl) };
    }, [url, config]);

    return { response, error, loading };
};

export const useHttpGetBlobPostponedExecution = <T = Blob, E = unknown, C = unknown>(
    config?: AxiosRequestConfig<C>
    ...
    const cleanUpBlob = (): void => { blobUrl && URL.revokeObjectURL(blobUrl) };
